...
const Web3Provider: FunctionComponent<Props> = ({children}) => {
 ...
 // Ex 18.95, Sign Contract 
 //added ganache account 0's private key to metamask first
        const signer = provider.getSigner();
        const signedContract = contract.connect(signer);
  //debugger
  // modify for es 21.108 fix reload of the brave browser
    setTimeout(() =>    setGlobalListeners(window.ethereum),500); 
        setWeb3Api(createWeb3State({
          ethereum: window.ethereum,
          provider,
      // bef ex 18.95   contract: contract as unknown as NftMarketContract,
